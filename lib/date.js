...
/** Copy locales */
function setup_locale(name, parent_name, opts) {
...
var _format_mapping = {
	...
	'd': function() { var d = this.getFormat('%{day}'); return with_leading_zero(d); },
	'e': function() { return '' + this.getFormat('%{day}'); },
...
helpers.setLocale = function(name) {
	_current_locale = name;
	return helpers;
};

/** Format time */
helpers.format = function(opts, time) {
	...
	opts = opts || {};
	opts.locale = opts.locale || _current_locale;
	format_str = '' + ( opts.format || format_str );
